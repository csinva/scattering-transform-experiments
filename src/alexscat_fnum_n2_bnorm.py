...
__all__ = ['alexscat_fnum_n2_bnorm']


class AlexScat_FNum_n2_bnorm(nn.Module):

    def __init__(self, num_classes=10, n = 32, j = 2, l = 2):
        super(AlexScat_FNum_n2_bnorm, self).__init__()
        self.J = j
        self.N = n
        self.L = l
        ...
        if self.nfscat*3 < self.n_flayer:
            self.first_layer = nn.Sequential(
                ...
                nn.BatchNorm2d(self.n_flayer - self.nfscat*3),
                )
        else:
...
def alexscat_fnum_n2_bnorm(**kwargs):
    """AlexNet model architecture from the
    `"One weird trick..." <https://arxiv.org/abs/1404.5997>`_ paper.
    """
    model = AlexScat_FNum_n2_bnorm(**kwargs)
    return model
