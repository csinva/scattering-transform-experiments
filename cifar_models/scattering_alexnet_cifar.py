...
#DEPENDS ON THE WHOLE NHWC THING, MAY NEED TO RESHAPE
M, N = x.shape.as_list()[-2:]
J = 4
scat = scattering.Scattering(M=M, N=N, J=J)(x)
#DEPENDS ON THE NHWC THING.
...
fc6 = tf.nn.relu_layer(tf.reshape(tf.concat(conv5, scat), [-1, int(prod(conv5.get_shape()[1:]) + prod(scat.get_shape()[1:]))]), fc6W, fc6b)
